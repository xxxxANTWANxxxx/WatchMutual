...
export class Tab3Page
{
  ...
  ionViewWillEnter()
  {
    //reload friends, because we always want to see friends, but not people we arent friends with
    this.loadFriends();
    this.search2 = this.users
  }
  //load friends into list to display friends
  loadFriends()
  {
    const httpOptions = {
    ...
    this.http.post("http://localhost:4200/tabs/tab3", postData, httpOptions)
      .subscribe(data =>
      {
        ...
        //list to hold all friends, so when we reset search, we still have the full list
        this.search2 = this.users
      }, error =>
        {
          console.log('failure')
  ...
  //same thing as load friends but for everyone in the db
  loadPeople()
  {
    const httpOptions = {
      headers: new HttpHeaders({
  ...
  searchthis(event)
  {
    /*when we search, we want to search both friends and people. 
    so this method calls both, with both do the same thing for thier respective attribute */
    this.searchFriends(event);
    this.searchPeople(event);
  }

  //filter people in search
  searchPeople(event)
  {
  ...
  //link to view a users profile(friend or person)
  viewProfile(i): void
  {
