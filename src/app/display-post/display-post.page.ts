import { Component, OnInit } from '@angular/core';
import { Router, NavigationExtras } from '@angular/router';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { AlertController } from '@ionic/angular';


@Component({
  selector: 'app-display-post',
  templateUrl: './display-post.page.html',
  styleUrls: ['./display-post.page.scss'],
})
export class DisplayPostPage implements OnInit
{
  ionViewWillEnter()
  {
    //reload posts every time page is entered
    this.loadpost()
  }

  arrayBufferToBase64(buffer)
  {
    var binary = '';
    var bytes = new Uint8Array(buffer);
    var len = bytes.byteLength;
    for (var i = 0; i < len; i++)
    {
      binary += String.fromCharCode(bytes[i]);
    }
    return window.btoa(binary);
  }

  private comment: string;
  private postId: number;
  private allData = [];
  private allComments = [];



  constructor(private http: HttpClient, private router: Router, public alertController: AlertController)
  {
    this.postId = this.router.getCurrentNavigation().extras.state.pid;
  }

  loadpost()
  {
    const httpOptions = {
      headers: new HttpHeaders({
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      })
    };

    this.http.post("http://localhost:4200/find-post", { "pid": this.postId }, httpOptions)
      .subscribe(data =>
      {
        this.allData = data['results']
        if (data['results'].user.pic != null)
          data['results'].user.pic = this.arrayBufferToBase64(data['results'].user.pic.data)
        else data['results'].user.pic = ""

        this.allComments = data['results'].comments
        for (let i = 0; i < data['results'].comments.length; i++)
          if (data['results'].comments[i].user.pic != null)
            data['results'].comments[i].user.pic = this.arrayBufferToBase64(data['results'].comments[i].user.pic.data)
          else data['results'].comments[i].user.pic = ""
      }, error =>
        {
          console.log(error)

        });
  }

  replyClicked()
  {
    //when user posts, calls app.js to add row to db with new post
    const httpOptions = {
      headers: new HttpHeaders({
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      })
    };
    let postData = {
      "comment": this.comment,
      "pid": this.postId
    }

    this.http.post("http://localhost:4200/reply", postData, httpOptions)
      .subscribe(data =>
      {
        this.comment = null;
        this.loadpost();

      }, error =>
        {
          console.log(error)

        });
  }

  ngOnInit()
  {
    this.loadpost()
  }

}
