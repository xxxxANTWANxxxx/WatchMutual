...
export class DisplayUserPage implements OnInit
{
  ...
  //load users info if page is displayed
  ionViewWillEnter()
  {
    this.loadUser();
  }

  //http post from app.js for users personal info
  loadUser()
  {
    const httpOptions = {
    ...
    //again, just the profile pic display method
  ...
  //actually adding the data to diaplay for inf-scroll
  addMoreItems()
  {
  ...
  //when inf-scroll reaches its end, more data is loaded from this
  loadData(event)
  {
    setTimeout(() =>
    {
  ...
  //toggles inf-scroll even attepting to load data
  toggleInfiniteScroll()
  {
    this.infiniteScroll.disabled = !this.infiniteScroll.disabled;
  }

  //display for if user becomes friends
  async becameFriends()
  {
  ...
  //display for if user already was friends
  ...
  //when user is an admin, he has access to the button that calls this, which confirms, and then deletes the account, including that users posts, lists, friendships, etc
  async deleteAccount()
  {
    const alert = await this.alertController.create({
      header: 'Alert',
  ...
  //actual http post to db to drop tables
  deleteUser()
  {
    const httpOptions = {
      headers: new HttpHeaders({
