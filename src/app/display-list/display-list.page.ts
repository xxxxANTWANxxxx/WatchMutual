...
@Component({
  ...
  templateUrl: './display-list.page.html',
  styleUrls: ['./display-list.page.scss'],
})
export class DisplayListPage implements OnInit
{
  ...
  {
    this.selectedUser = this.router.getCurrentNavigation().extras.state.uid;
  ...
  viewLists()
  {
    ...
    this.http.post("http://localhost:4200/search-user-lists", postData, httpOptions)
      .subscribe(tdata =>
      {
        ...
        for (let i = 0; i < this.allData.length; i++)
        {
          if (this.allData[i].user.pic != null)
            this.allData[i].user.pic = this.arrayBufferToBase64(this.allData[i].user.pic.data)
